...
// orphan_models_wrt_world: vector of models that are seen in the box, but DO NOT belong in the box
  ...
  for(int i=0;i<num_parts_seen-1;i++) {
	 ...
	 if(desired_models_wrt_world[j].type.compare(image_models_wrt_world[i].type)==0){
            ...
            if(within_spec(difference_pose) && !i_catagorized[i]  && !d_catagorized[j]){
 ...
 for(int i=0;i<num_parts_seen-1;i++) {
    if(!i_catagorized[i]){
      model = image_models_wrt_world[i];
 ...
 for(int i=0;i<num_parts_desired;i++) {
    if(!d_catagorized[i]){
        ...
        string model_name(model.type);
        if (model_name != box_name) {  //If not a shipping box add to missing_models
